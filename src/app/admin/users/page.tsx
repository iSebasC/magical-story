...
import { Search, UserPlus, Star, Sprout, Pencil, Trash2, X, CheckCircle, AlertCircle } from 'lucide-react';
    ...
    <div className="p-5 lg:p-7 relative">
      ...
      {showCreateForm && (
        <div className="bg-white rounded-2xl border border-cream2 p-6 mb-4">
          <div className="font-display font-medium text-ink mb-4 tracking-wide flex justify-between">
            <span className="flex items-center gap-2"><UserPlus className="w-4 h-4" /> Create new user</span>
            <button onClick={cancelCreateUser} className="text-inkm hover:text-ink"><X className="w-4 h-4" /></button>
          </div>
          ...
          <div className="flex gap-2.5">
            <button onClick={createUser} disabled={isSubmitting} className="px-5 py-2 rounded-full text-sm font-medium text-white bg-orange hover:bg-oranged transition-all shadow-[0_3px_10px_rgba(255,107,53,.3)] disabled:opacity-50">
              {isSubmitting ? 'Creating...' : <><CheckCircle className="w-3.5 h-3.5 inline -mt-0.5" /> Create user</>}
            </button>
          </div>
        </div>
      )}
                  ...
                  <tr key={user.id} className="hover:bg-[#F8F5F0] transition-colors">
                    ...
                    <td className="px-5 py-3.5 text-center">
                      <button onClick={() => toggleUserPlan(user.id, user.plan, user.name)} className={`text-[10px] font-medium px-2.5 py-1 rounded-full border transition-all ${user.plan === 'premium' ? 'bg-orange/12 text-oranged border-orange/20 hover:bg-red-50 hover:text-red-500 hover:border-red-200' : 'bg-cream text-inkm border-cream2 hover:bg-mint/10 hover:text-green-700 hover:border-mint/40'}`}>
                        {user.plan === 'premium' ? <><Star className="w-3 h-3 inline -mt-0.5" /> Premium</> : <><Sprout className="w-3 h-3 inline -mt-0.5" /> Free</>}
                      </button>
                    </td>
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex justify-end gap-2">
                        <button onClick={() => openEditModal(user)} className="px-2.5 py-1 rounded-lg text-[11px] font-medium text-inks bg-cream border border-cream2 hover:border-orange hover:text-orange transition-all flex items-center gap-1">
                          <Pencil className="w-3 h-3" /> Edit
                        </button>
                        <button onClick={() => deleteUser(user.id, user.name)} className="px-2.5 py-1 rounded-lg text-[11px] font-medium text-red-500 bg-red-50 border border-red-100 hover:bg-red-500 hover:text-white transition-all flex items-center gap-1">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/40 backdrop-blur-sm">
          <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-cream2 flex justify-between items-center bg-cream/50">
              ...
              <button onClick={closeEditModal} className="text-inkm hover:text-ink w-8 h-8 flex items-center justify-center rounded-full hover:bg-cream transition-colors"><X className="w-4 h-4" /></button>
            </div>
      ...
      {toastVisible && (
        <div className="fixed top-5 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 bg-ink text-white text-sm font-medium px-5 py-3.5 rounded-2xl shadow-xl border-l-4 border-mint animate-in fade-in slide-in-from-top-4 duration-300">
          <span>{toastMessage.includes('Error') || toastMessage.includes('Failed') ? <AlertCircle className="w-4 h-4 text-red-400" /> : <CheckCircle className="w-4 h-4 text-mint" />}</span>
